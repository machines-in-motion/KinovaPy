...
while time.perf_counter()-start_time < run_time:
  if device:
    ...
    if not (state.x==0 and state.y==0 and state.z==0 and state.roll==0 and state.pitch==0 and state.yaw==0):
        reach_pos = np.array([ state.x, state.y, state.z])  # x, y, z
        reach_rot = pin.rpy.rpyToMatrix(np.array([state.roll, state.pitch, state.yaw]))   # roll, pich, yaw
        reach_pose = pin.SE3(reach_rot, reach_pos)
  ...
  if not in_bounds(reach_pose.translation, pos_min, pos_max):
