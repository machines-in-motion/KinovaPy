...
while time.perf_counter()-start_time < run_time:
  if device:
    state = device.read()
    if (state.x==0 and state.y==0 and state.z==0 and state.roll==0 and state.pitch==0 and state.yaw==0)==False:
      ...
      #pose.translation += np.array([0.3, 0., 0.3])
      #goals = [pose]
